...
resolveJoin({belongs_to, Remote, Field}, Table, Record) -> find(Remote, maps:get(Field, Record));
resolveJoin({has_many,   Remote, Field}, Table, Record) -> grep(Remote, #{Field => maps:get(Field, Record)}).
